...
io.on('connection',(socket)=>{
    console.log('New SocketIO connection detected.')
    ...
    socket.on('sendMessage',(message,callback)=>{
        ...
        if(filter.isProfane(message)){
            return callback('Profanity detected in message.')
        }
    ...
    socket.on('disconnect',()=>{
        ...
        if(user){
        io.to(user.room).emit('message',generateMsg('Admin',`${user.username} has disconnected.`))
        io.to(user.room).emit('roomData',{
            room: user.room,
            users: usersInRoom(user.room)
        })
